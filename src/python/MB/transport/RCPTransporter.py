...
Transporter implementation for using rcp

"""
__version__ = "$Version$"
__revision__ = "$Id: RCPTransporter.py,v 1.1 2005/12/30 18:51:41 evansde Exp $"
...
class RCPTransporter(Transporter):
    """
    _RCPTransporter_
    
    Specialisation of Transporter to handle transfers
    via rcp commands
    """
    ...
    def transportIn(self, mbInstance):
        ...
        try:
            ...
            raise ex
        mbInstance['URL'] = "rcp://%s" % mbInstance['RemoteName']
        return
    ...
    def transportOut(self, mbInstance):
        ...
        try:
            ...
            ex.addInfo(MetaBroker = mbInstance)
            raise ex
        mbInstance['URL'] = "rcp://%s" % mbInstance['Target']
        return
    ...
    def _ExtractOpts(self, mbInstance):
        """
        Extract any extra options for the rcp command
        from the metabroker and construct the args
        dictionary
        """
        args = {}
        if mbInstance.has_key("RcpBinary"):
            args['Binary'] = mbInstance['RcpBinary']
        if mbInstance.has_key("RcpOptions"):
            args['Options'] = mbInstance['RcpOptions']
        if mbInstance.has_key('RcpTimeout'):
            args['Timeout'] = mbInstance['RcpTimeout']
        else:
            args['Timeout'] = 5
        return args
    ...
    def _Copy(self, source, target, **args):
        """
        Construct and execute the rcp command,
        and check the exit code for success.
        A timeout is included since non kerberised
        rcp can hang for a long time
        """
        binary = args.get('Binary','rcp')
        ...
        while pop.poll() == -1:
            ...
            if timediff > timeout:
                msg = "Rcp Transport timed out after "
                msg += "%s seconds\n" % timeout
                msg += "Command Used: %s" % comm
                raise TransportFailed(
                    msg,
                    ModuleName = "MB.transport.RCPTransporter",
                    ClassName = "RCPTransporter",
                    MethodName = "_Copy")
        exitCode = pop.poll()
        del pop
        if exitCode:
