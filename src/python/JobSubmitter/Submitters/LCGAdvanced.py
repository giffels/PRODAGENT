...
class LCGAdvanced(SubmitterInterface):
    """
    _LCGAdvanced_
...
##         script.append("cp %s/*/*.root .\n" % jobname )
    ...
    def doSubmit(self, wrapperScript, jobTarball):
        ...
        logging.debug( "LCGAdvanced.doSubmit bossJobId = %s"%bossJobId)
        if bossJobId==0:
        ...
        # logging.info( "LCGAdvanced.SubmitCommand: %s", bossSubmit)
        output = BOSSCommands.executeCommand(bossSubmit)
        logging.debug ("LCGAdvanced.doSubmit: %s" % output)
    ...
    def editDashboardInfo(self, dashboardInfo):
        ...
        for i in self.parameters['Whitelist']:
        	try:
        		tmp=int(i)
        		targetce.append(semap[tmp])
        	except:
        		targetce.append(i)
        dashboardInfo['TargetCE'] = self.listToString(targetce)
...
registerSubmitter(LCGAdvanced, LCGAdvanced.__name__)
