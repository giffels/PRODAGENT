...
__revision__ = "$Id: FNALLPCCAF.py,v 1.10 2008/10/21 15:52:21 gutsche Exp $"
...
class FNALLPCCAF(BulkSubmitterInterface):
    """
    _FNALLPCCAF_
    ...
    def doSubmit(self):
        ...
        logging.debug("FNALLPCCAF.doSubmit: %s" % condorSubmit)
        output = self.executeCommand(condorSubmit)
        logging.info("FNALLPCCAF.doSubmit: %s " % output)
        return
    ...
    def initJDL(self):
        ...
        jdl.append('Requirements          = Memory >= 199 && OpSys == "LINUX" && (Arch != "DUMMY")\n')
...
registerSubmitter(FNALLPCCAF, FNALLPCCAF.__name__)
