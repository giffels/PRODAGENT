<?php
  ...
  # Finished jobs page of the the Production Agent web interface
  # Also main page
  #
  ...
  echo "<h2>Select finished jobs:</h2><br>";

  // use user selected parameters ...
  if (isset($_GET['submit'])) {

    // get number of hours of finished required status
    $finished = $_GET['finished_input'];
    switch ($finished) {
      case 1:
        $hours = 1;
        break;
  ...
  // select finished time
  print_select('finished_input',
       'Finished on: ',
  ...
  if (isset($_GET['submit'])) {
    ...
    echo "<h2>Finished jobs information:</h2><br>";
    ...
    if ($cmssw) {
      ...
      $query = "select JOB.TASK_ID as id, JOB.SUB_TIME as subTime, " .
         ...
         (($finished == 0) ? "" : " and STOP_T > (UNIX_TIMESTAMP() - " .
                                 (3600 * $hours) . ")") . " " .
      ...
      while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
        ...
        if ($headers_printed == 0) {
          ...
          // print table header
          start_table();
          table_headers(array('Job id','Submission date','Finished time',
