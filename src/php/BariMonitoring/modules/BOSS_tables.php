...
for ($i=0;$i<count($task_name);$i++){			
	if($DB_type==2){
		if($task_name[$i]=="Success")$output=$SuccessArchive;
		if($task_name[$i]=="Failed")$output=$FailureArchive;
	}
...
if($DB_type==2){
		if($task_name[$i]=="Success")
			echo "<td>&nbsp;$task_exit[$i]</td><td><a href=\"Production/$output/$retrive_file[$i].tar.gz\">$task_id[$i]</a></td>";
		if($task_name[$i]=="Failed"){
			preg_match("/\d{6,}/",$retrive_file[$i],$matches);
			$id=$matches[0];
			echo "<td>&nbsp;$task_exit[$i]</td><td><a href=\"Production/$output/$retrive_file[$i]-cache/JobTracking/Failed/\">$task_id[$i]</a></td>";
			}
}
else
                echo "<td>&nbsp;$task_exit[$i]</td><td><a href=\"Production/$prodarea_alias/JobTracking/BossJob_$task_id[$i]_1/Submission_$id[$i]\">$task_id[$i]</a></td>";
...
<?php
if($DB_type==2){
		echo "<td><a href=\"Production/$output\">$task_id[$i]</a></td>";
}
else
                echo "<td><a href=\"Production/$prodarea_alias/JobTracking/BossJob_$task_id[$i]_1/Submission_$id[$i]\">$task_id[$i]</a></td>";
?>
<td><?=$id[$i]?></td>
<td><?=$job_name[$i]?></td>
<td>&nbsp;<?=$comment[$i]?></td>
