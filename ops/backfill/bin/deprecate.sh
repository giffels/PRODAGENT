...
mysql -u root -D ProdAgentDB -p98passwd -S ${PRODAGENT_ROOT}/workdir/mysql/sock -B -e "update we_Job set retries=3 where workflow_id regexp '*$site*';"
...
for wf in $wfs; do 
    ...
    #mysql -u root -DProdAgentDB -p98passwd -S $PRODAGENT_ROOT/workdir/mysql/sock -e "update jq_queue set workflow_id='$new_wf' where workflow_id='$wf'";
